// sds32_two_sboxes_binary_io_independent.c  (C99)
// Input : 32 chars '0'/'1' (MSB first)
// Output: 32 chars '0'/'1' (MSB first)
//
// build:
//   gcc -std=c99 -O2 SDS.c -o sds32

// sds32_clean_with_mds.c  (C99)
// Input : 32 chars '0'/'1' (MSB first)
// Output: 32 chars '0'/'1' (MSB first)
//
// build:
//   gcc -std=c99 -O2 sds32_clean_with_mds.c -o sds32

#include <stdint.h>
#include <stdio.h>
#include <string.h>

//
// -------------------- S-boxes --------------------
//

static const uint8_t SBOX_A[256] = {
    
};

static const uint8_t SBOX_B[256] = {
    
};

//
// -------------------- S-box selection --------------------
//

static const uint8_t *SBOX_PRE[4]  = { SBOX_A, SBOX_B, SBOX_A, SBOX_B };
static const uint8_t *SBOX_POST[4] = { SBOX_B, SBOX_A, SBOX_B, SBOX_A };

static inline uint8_t apply_sbox(const uint8_t *S, uint8_t x) {
    return S[x];
}

//
// -------------------- MDS diffusion (AES MixColumns) --------------------
//

//*2
static inline uint8_t xtime(uint8_t b) {
    return (uint8_t)((b << 1) ^ ((b & 0x80u) ? 0x1Bu : 0x00u));
}

static inline void diffusion_mds(uint8_t b[4]) {
    uint8_t s0 = b[0], s1 = b[1], s2 = b[2], s3 = b[3];

    b[0] = (uint8_t)(xtime(s0) ^ (xtime(s1) ^ s1) ^ s2 ^ s3);
    b[1] = (uint8_t)(s0 ^ xtime(s1) ^ (xtime(s2) ^ s2) ^ s3);
    b[2] = (uint8_t)(s0 ^ s1 ^ xtime(s2) ^ (xtime(s3) ^ s3));
    b[3] = (uint8_t)((xtime(s0) ^ s0) ^ s1 ^ s2 ^ xtime(s3));
}

//
// -------------------- One round --------------------
//

void sds32_round(uint8_t b[4]) {
    for (int i = 0; i < 4; i++)
        b[i] = apply_sbox(SBOX_PRE[i], b[i]);

    diffusion_mds(b);

    for (int i = 0; i < 4; i++)
        b[i] = apply_sbox(SBOX_POST[i], b[i]);
}

//
// -------------------- Binary I/O --------------------
//

static int parse_bin32(const char *s, uint8_t b[4]) {
    if (strlen(s) != 32) return 0;

    for (int i = 0; i < 4; i++) {
        uint8_t v = 0;
        for (int j = 0; j < 8; j++) {
            char c = s[i * 8 + j];
            if (c != '0' && c != '1') return 0;
            v = (uint8_t)((v << 1) | (c - '0'));
        }
        b[i] = v;
    }
    return 1;
}

static void print_bin32(const uint8_t b[4]) {
    for (int i = 0; i < 4; i++)
        for (int j = 7; j >= 0; j--)
            putchar(((b[i] >> j) & 1u) ? '1' : '0');
    putchar('\n');
}

//
// -------------------- main --------------------
//

// int main(void) {
//     char in[64];
//     uint8_t state[4];

//     if (scanf("%63s", in) != 1) return 1;
//     if (!parse_bin32(in, state)) return 1;

//     sds32_round(state);

//     print_bin32(state);
//     return 0;
// }
